...
void read(queue<int> &buf, int speed_time) {
    while (true) {
        ...
        if (!buf.empty()) {
            ...
            cout << "READER ("<<this_thread::get_id()<<") read number: " << number << endl;
            //Выводим заполненность буфера после его чтения
            cout<<"Bufer size = "<<counter<<endl;
        }
        //открываем разделяемые ресурсы для других потоков
        mtx.unlock();
